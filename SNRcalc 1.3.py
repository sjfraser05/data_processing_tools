...
outMsg ="Do you want to remove outliers?"
outTitle = "Outlier Removal"
outChoices = ["Yes - 3\u03C3", "No outlier removal"]
outChoice = easygui.choicebox(outMsg, outTitle, outChoices)

if outChoice == "Yes - 3\u03C3":
    distance_from_mean = abs(areaList - np.mean(areaList))
    max_deviations = 3
    not_outlier = distance_from_mean < max_deviations * np.std(areaList)
    areaList = areaList[not_outlier]
    SNR = np.mean(areaList) / np.std(areaList)
    RSD = np.std(areaList) / np.mean(areaList) * 100
...
plt.show()
figureImageName = str(directory.split("\\")[-1]) + ".png"
fig.savefig(figureImageName, dpi=100)

print(areaList)

if MatChoice == "new custom":
